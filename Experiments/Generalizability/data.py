...
def load_SVHN(download=False):
    ...
    trainset = datasets.SVHN(
        "SVHN/", split="train", download=download, transform=svhn_transform
    )
    testset = datasets.SVHN(
        "SVHN/", split="test", download=download, transform=svhn_transform
    )
...
def load_CIFAR_cats_dogs(download=False):
    ...
    trainset = datasets.CIFAR10(
        root="CIFAR/", train=True, download=download, transform=cifar_transform
    )
    testset = datasets.CIFAR10(
        root="CIFAR/", train=False, download=download, transform=cifar_transform
    )
...
def load_Kaggle_cats_dogs():
    ...
    train_data, test_data, train_label, test_label = train_test_split(
        dataset.imgs, dataset.targets, test_size=0.05, random_state=42
    )
    train_transform = transforms.Compose(
        [transforms.Resize((32, 32)), transforms.ToTensor(), transforms.Grayscale()]
    )
