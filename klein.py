...
def klein(y: int, x: int, theta1: float, theta2: float) -> float:
    return np.sin(theta2) * (np.cos(theta1) * x + np.sin(theta1) * y) + np.cos(
        theta2
    ) * Q(np.cos(theta1) * x + np.sin(theta1) * y)
...
def generate_pc_filter(size: int, theta1: float) -> np.ndarray:
    ...
    for x in range(size):
        for y in range(size):
            ...
            Filter = dblquad(primary_circle, xmin, xmax, ymin, ymax, args=((theta1,)))[
                0
            ]
    ...
    if not circle:
        for theta1 in thetas:
            for theta2 in thetas2:
                ...
                ax.set_title(
                    "Theta1: {}\u03c0 and Theta2: {}\u03c0".format(
                        theta1 / np.pi, theta2 / np.pi
                    )
                )
                plt.savefig(
                    "fig/th1-{}\u03c0_th2-{}\u03c0.pdf".format(
                        theta1 / np.pi, theta2 / np.pi
                    )
                )
                plt.show()
        ...
        for theta1 in thetas:
            ...
            ax = sns.heatmap(kfilter, cmap=cm.gray)
            ax.set_title(
                "Theta1: {}\u03c0 and Theta2: {}\u03c0".format(theta1 / np.pi, 0.5)
            )
            plt.savefig(
                "fig/th1-{}\u03c0_th2-{}\u03c0.pdf".format(theta1 / np.pi, 0.5)
            )
            plt.show()
